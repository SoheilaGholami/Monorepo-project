      ...
      rules: {
        'header-match-team-pattern': (
          ...
          if (!parsed) {
            return [
              false,
              'Unable to parse the commit message. Check the format.',
            ];
          }
          ...
          if (!app || !type || !subject) {
            return [
              false,
              "Commit message must follow the format '[APP-NAME] <type> (scope): subject'",
            ];
          }
          ...
          if (!scope) {
            return [
              true,
              'Commit message without scope is allowed but recommended.',
            ];
          }
